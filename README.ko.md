...
> **정식 릴리즈 — v0.2.0.** 실제 사용을 위해 배포하는 버전이며, 모든 Windows 앱과의 호환성은 계속 확인이 필요합니다.
...
1. [v0.2.0 릴리즈](https://github.com/ai-blink/pinte/releases/tag/v0.2.0)에서 `Pinte-v0.2.0-win-x64.zip`을 내려받습니다.
...
- 화면 캡처 숨김은 기본으로 꺼져 있으며 설정에서 바꿀 수 있습니다. 켜면 Pinte 보조 창을 화면 캡처에서 제외해 렌즈가 끝없이 겹쳐 보이는 피드백 루프를 막습니다.
...
`v0.2.0`은 입력 타이밍 고급 설정, 즉시 클릭을 놓치는 대상 앱을 위한 순서 있는 클릭 전달, 툴바의 렌즈 크기 조절 토글, 좁은 폭에서도 한 줄을 유지하는 툴바를 추가합니다. 핵심 클릭·드래그 흐름에는 자동 회귀 테스트가 있지만, 실제 대상 앱 동작은 사용하는 Windows 환경에서 계속 확인이 필요합니다. 재현 가능한 문제는 [GitHub Issues](https://github.com/ai-blink/pinte/issues)에 알려 주세요.
