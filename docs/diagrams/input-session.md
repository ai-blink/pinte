...
stateDiagram-v2
    [*] --> Disabled
    Disabled --> Pending: 확대 열기·조작 재개 / 요청 켜기
...
- 이 상태도는 구현을 설명한다. native 6개는 통과했지만 드래그 중 이중 포인터는 BLOCKED다. 정밀 경로·마우스 복귀 등 나머지 수락은 NEEDS_USER_UI_CHECK다. [최신 검증](../../notes/runs/2026-09-10-cursor-position-diagnostic.md)을 따른다.
