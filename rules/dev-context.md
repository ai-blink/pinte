...
- 2026-09-15 훅 워치독 회귀 수정(D-028): D-027이 상시 주입 커서를 훅 죽음으로 오판해 폭주(재설치 1000+)하고, 두 인스턴스 동시 실행이 훅 체인을 서로 느리게 하며, 누름 중 재설치가 드래그를 끊었다(로그 확정). 단일 인스턴스 Mutex + 재설치 전 태그 프로브 자기검증 + 진행 중 입력 보호 + 재설치 상한 12로 수정. 두 번째 실행 거부(exit 5)·새 프로세스 진입 창 재설치 0 실증. 실제 드래그·클릭은 `NEEDS_USER_UI_CHECK`. [기록](../notes/runs/2026-09-15-hook-watchdog-storm-regression.md)
