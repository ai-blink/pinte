...
- 2026-09-16 훅 복구 후속(D-029): D-028의 누적 12회 상한은 추가 훅 소실 뒤 복구를 영구 포기할 수 있어, 60초 이상 복구 시도가 끊긴 경우에만 예산을 초기화한다. 훅 소실 중 relay가 누름을 소유하면 물리 release 뒤 합성 Up 한 번과 stop으로 끝낸다. `relay-path`는 세션 armed·훅 callback·명령 큐·대상 Down 경계만 기록한다. key-demo-osk의 직접 원인은 미확정이며, `C:\app`에 path-trace publish를 교체·재실행했다. 실제 재현은 `NEEDS_USER_UI_CHECK`. [기록](../notes/runs/2026-09-16-hook-recovery-path-trace.md)
