...
- 2026-09-16: Pinte 전체 relay 무응답 재발을 조사했다. D-028의 누적 12회 복구 상한이 추가 훅 소실 뒤 영구 무응답을 만들 수 있어, 60초 이상 복구 시도가 끊긴 경우에만 예산을 초기화하도록 바꿨다. 훅 소실 중 relay가 누름을 소유하면 물리 버튼 해제 뒤 합성 Up 한 번과 stop으로 끝낸다. key-demo-osk는 `CallNextHookEx`를 반환하므로 직접 원인으로 확정하지 않았으며, 세션 armed·훅 콜백·명령 큐·대상 Down 경계를 남기는 저비용 `relay-path` 진단을 추가했다. build 0/0·총 134 테스트 통과, 새 publish 단일 파일을 `C:\app`에 교체·재실행했다. 실제 재현 결과는 `NEEDS_USER_UI_CHECK`. [기록](../notes/runs/2026-09-16-hook-recovery-path-trace.md).
