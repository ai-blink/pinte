...
- [ ] 설정 모달 후속 확인: 초기화 순서 오류 수정·표준 build 0/0, 회귀4사례 컴파일만 완료. 진입 창/일반·컴팩트 설정 열기, 메뉴 전환·변경 반영·닫은 뒤 재개는 NEEDS_USER_UI_CHECK. [기록](../notes/runs/2026-09-12-settings-modal-init-fix.md).

- [ ] 첫 프레임 무한 대기 후속 확인: 순환 대기 수정·표준 build 0/0, 회귀3사례 컴파일·실행 안 함. 사용자 재확인 대상은 최초 확대/영역 편집 후 첫 화면과 클릭·드래그다. [기록](../notes/runs/2026-09-12-first-frame-wait-fix.md).

- [ ] 컴팩트 렌즈·원본 표시 정책: Hidden 기본/5초/항상 윤곽선, 재편집→완료, 공통 ToolbarPlacement, 실제 8방향 렌즈 리사이즈 구현. 최종 build 경고0/오류0, 신규53사례 컴파일·실행 안 함. 코드/회귀 테스트와 표준 빌드 상태는 [이번 기록](../notes/runs/2026-09-12-compact-lens-source-indicator.md). 테스트 실행·UI·대상 반응은 사용자 확인 대기, NEEDS_USER_UI_CHECK. 다음은 [수동 행렬](../doc/compact-lens-validation.md) 결과에서 확인된 차단 수정 (2026-09-12).
- 이번 작업은 기존 dirty 보존 및 커밋·스테이징·푸시·브랜치 생성 금지다. 아래 과거 커밋 계획과 구분한다.
...
- 저장 기준: 영역 지정·드래그 수정은 main `dd0feb0`으로 저장 완료. UI 목업·계획·인계와 이번 시각·탐색 구현은 사용자 요청으로 로컬 커밋한다. 이전 실패 기록은 보존하며 푸시는 별도 요청 시 수행한다.
