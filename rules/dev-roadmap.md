...
- [ ] 렌즈 배치·접기 후속 확인: 화면 밖 배치 보정 제거, 넓은 8방향 실시간 리사이즈와 상단 이동 손잡이 우선, 클릭 지점 `⌕` 펼치기 오버레이, 좁은 선택 도구막대 압축, 0.1× 버튼·일반/컴팩트 직접 배율 입력을 반영했다. build 0/0·총 143 테스트 통과. 다음은 화면 밖 위치·모서리 리사이즈·이동 hover·아이콘 펼치기·직접 입력·최소 선택 폭의 `NEEDS_USER_UI_CHECK`다 (2026-09-17).
